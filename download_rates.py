...
rates_filename = "rates-last90d.xml"
...
def getRateInEUR(billDate, billCurrency):
    #return 1 if currency is EUR
    if billCurrency == "EUR":
        return 1.0

    xml_rates = minidom.parse(rates_filename)
...
def cleanUpRates():
    try:
        with open(rates_filename):
            remove(rates_filename)
...
def receiveECBRates():
    ...
    try:
        urllib.urlretrieve(ecbURL, rates_filename)
        return 0
    except IOError:
